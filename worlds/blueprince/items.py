...
def create_all_items(world: BluePrinceWorld) -> None:
    ...
    if world.options.room_draft_sanity:
        world.starting_rooms = world.random.choices(
            [room for room in room_item_list if ROOM_PICK_POSITIONS_KEY in rooms[room.name] and (set(rooms[room.name][ROOM_PICK_POSITIONS_KEY]) & ENTRANCE_HALL_DRAFTABLE) and not (room.name == "Gift Shop" and world.options.goal_type.value <= 1)],
